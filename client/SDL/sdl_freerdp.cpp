...
static BOOL sdl_create_windows(SdlContext* sdl)
{
	...
	const char* title = sdl_window_get_title(sdl->context()->settings);
	BOOL rc = FALSE;
...
static DWORD WINAPI sdl_client_thread_proc(SdlContext* sdl)
{
	...
	freerdp* instance = sdl->context()->instance;
	WINPR_ASSERT(instance);
...
SdlContext::SdlContext(rdpContext* context)
    : _context(context), log(WLog_Get(SDL_TAG)), update_complete(true), disp(this), input(this),
      clip(nullptr), primary(nullptr, SDL_FreeSurface), primary_format(nullptr, SDL_FreeFormat)
{
}
